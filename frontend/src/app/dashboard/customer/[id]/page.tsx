...
export default function CustomerPage({ params }: {
	...
	const [searchQueryCashProceeds, setSearchQueryCashProceeds] = useState('')
	...
	const filteredDetails = customer.details?.filter((detail) =>
		...
		detail.payment.name.toString().toLowerCase().includes(searchQueryDetail.toLowerCase()));
	...
	const filteredInvoices = customer.invoices?.filter((invoice) =>
		...
		|| invoice.payment.name.toString().toLowerCase().includes(searchQueryInvoice.toLowerCase()));

	const filteredCashProceeds = [
		...(customer.invoices?.flatMap((invoice) =>
			invoice.cashProceeds?.filter((cashProceed) =>
				cashProceed.issueDate.toString().toLowerCase().includes(searchQueryInvoice.toLowerCase())
				|| cashProceed.amount.toString().includes(searchQueryInvoice)
				|| cashProceed.paymentType.name.toString().toLowerCase().includes(searchQueryInvoice.toLowerCase())
				|| cashProceed.operation.name.toString().toLowerCase().includes(searchQueryInvoice.toLowerCase())
				|| cashProceed.description.toString().toLowerCase().includes(searchQueryInvoice.toLowerCase())
			) || []
		) || []),
		...(customer.details?.flatMap((detail) =>
			detail.cashProceeds?.filter((cashProceed) =>
				cashProceed.issueDate.toString().toLowerCase().includes(searchQueryInvoice.toLowerCase())
				|| cashProceed.amount.toString().includes(searchQueryInvoice)
				|| cashProceed.paymentType.name.toString().toLowerCase().includes(searchQueryInvoice.toLowerCase())
				|| cashProceed.operation.name.toString().toLowerCase().includes(searchQueryInvoice.toLowerCase())
					...
					<div className="space-y-6">
						...
						<div className="flex gap-4 relative">
							<div className="flex items-center space-x-2 w-[28rem] max-w-md min-w-min">
								...
								<Input
										placeholder={activeView === "invoices"
											? "Search Invoices..."
											: activeView === "sales"
												? "Search Sales Transaction..."
												: "Search Cash Proceeds..."}
									className="flex-1"  // Ensure Input takes remaining space
									value={activeView === "invoices" ? searchQueryInvoice : activeView === "sales" ? searchQueryDetail : searchQueryCashProceeds}
									onChange={(e) => activeView === "invoices" ? setSearchQueryInvoice(e.target.value) : activeView === "sales" ? setSearchQueryDetail(e.target.value) : setSearchQueryCashProceeds(e.target.value) }
								/>
							</div>
									...
									<Calendar
										...
										onSelect={(date) => {
											...
											if (activeView === "invoices") {
												...
												setSearchQueryCashProceeds(formattedDate);
											}
										}}
									/>
								</PopoverContent>
						...
						<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
							<Card className="p-4">
								...
								) : filteredCashProceeds.length }</div>
							</Card>
						...
						<div>
							<Select onValueChange={(value) => setActiveView(value)} defaultValue="invoices">
								...
								<SelectContent className="bg-gradient-to-r from-indigo-700 to-indigo-950 text-white">
									...
									<SelectItem
										className="flex justify-center hover:bg-indigo-950 focus:bg-indigo-950 focus:text-white"
										value="sales">Sales Transaction</SelectItem>
									<SelectItem
										className="flex justify-center hover:bg-indigo-950 focus:bg-indigo-950 focus:text-white"
										value="cashproceeds">Cash Proceeds
									</SelectItem>
								</SelectContent>
							</Select>
						</div>
							...
							<ScrollArea className="h-[600px]">
								{activeView === "invoices" ? (
											...
											{filteredInvoices.map((invoice) => (
												<React.Fragment key={invoice.id}>
													...
													>
														...
														<TableCell><TypeBadge type={invoice.payment}/></TableCell>
										...
										{filteredDetails.map((detail) => (
											<React.Fragment key={detail.id}>
												...
												>
													...
													<TableCell><TypeBadge type={detail.payment}/></TableCell>
									...
									<Table>
										...
										<TableBody>
											{filteredCashProceeds.map((cashProceed) => (
												<TableRow key={cashProceed.id}>
													<TableCell>{cashProceed.invoiceId ? filteredInvoices.find((invoice) => invoice.id === cashProceed.invoiceId)?.invoiceNumber + " Invoice" : filteredDetails.find((detail) => detail.id === cashProceed.customerDetailId)?.name + " Sales"}</TableCell>
													<TableCell>{cashProceed.issueDate}</TableCell>
													<TableCell>{cashProceed.amount}</TableCell>
													<TableCell><StatusBadge status={cashProceed.paymentType}/></TableCell>
													<TableCell><TypeBadge type={cashProceed.operation}/></TableCell>
													<TableCell>{cashProceed.description}</TableCell>
													<TableCell className="text-right">
														<div className="flex items-center justify-end">
															<DeleteButton
																onDelete={async () => handleDelete(cashProceed.id, "/CashProceed/CashProceedsDelete", "Cash Proceeds")}
																description="This action cannot be undone. This will permanently delete the cash proceeds and remove all associated data."
															/>
														</div>
													</TableCell>
												</TableRow>
